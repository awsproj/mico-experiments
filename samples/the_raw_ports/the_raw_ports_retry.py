...
def test_ports_packet():
    ...
    while True: # scope
        ...
        for k in range(batch1 * batch2):
            ...
            rx_hdr = "00%02d%04ddsz:%04d" % (seqn, pktsz, pktsz)
            retry_ok = False
            for r in range(4): # retry count
                ...
                if cmd_ok and ack_ok:
                    retv_cmd.append([-2, -2, "OK"])
                    retry_ok = True
                elif ack_ok:
                    retv_cmd.append([-2, -2, "OK-ack"])
                    retry_ok = True
                elif cmd_ok:
                    retv_cmd.append([-2, -2, "Retry-cmd-ok"])
                else:
                    retv_cmd.append([-2, -2, "Retry-no-cmd-ok"])
